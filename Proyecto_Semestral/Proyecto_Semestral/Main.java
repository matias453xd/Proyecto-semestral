...
public class Main {
    ...
    public static Venta VenderPasajes(Scanner input, Viaje[] viajes1, Bus[] buses1, Cliente[][] pasajeros1, Venta[] ventaa, String[] patente){
        ...
        String fecha = input.next();
        //En vez de preguntar el nombre, seria mejor mostrar 
        //un catalogo de clientes y de ahí elegir uno.
        System.out.print("Nombre: ");//aqui
        ...
        for(int j = 0; j < ventaa.length; j++){
            if(ventaa[j] == null){//El error debe esta aca
                ...
                if(buses1[elegir].getNumeroAsientos()>= boletos){
                    buses1[elegir].setNumeroAsientos(buses1[elegir].getNumeroAsientos() - boletos);
                } else {
                    System.out.println("No hay suficientes asientos ");
                    return null;
                }
                System.out.println("Asientos disponibles para el viaje seleccionado: ");
                buses1[elegir].mostrarMatriz(buses1[elegir].getFilas(),buses1[elegir].getColumnas());
                for(int d = 0; d < boletos; d++){
                    System.out.println("Elija un asiento indicando su numero: ");
                    String num = input.next();
                    int numeroEntero = Integer.parseInt(num);
                    if(numeroEntero > 20 || numeroEntero < 0){
                        while(numeroEntero > 20 || numeroEntero < 0){
                            System.out.println("Elija un asiento valido: ");
                            num = input.next();
                            numeroEntero = Integer.parseInt(num);
                        }
                    }
                    buses1[elegir].setMatriz(num, buses1[elegir].getFilas(), buses1[elegir].getColumnas());
                    buses1[elegir].mostrarMatriz(buses1[elegir].getFilas(),buses1[elegir].getColumnas());
                }
